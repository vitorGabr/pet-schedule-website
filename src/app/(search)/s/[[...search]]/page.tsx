...
import {
  pageSearchLoader,
  PageSearchParams,
} from "@/schemas/page-search-params";
  ...
  return (
    <div className="bg-muted/50 px-5 md:px-10 py-4 md:py-8">
      ...
      <Suspense fallback={<CompanyCardSkeletonGrid count={6} />}>
        <Content search={search} searchParams={searchParams} />
      </Suspense>
    </div>
  );
}
...
async function Content(props: {
  ...
  const data = await searchCompanies({
    search: props.searchParams.q?.trim(),
    categories: props.search,
    page: props.searchParams.page,
  });

  return (
    <div>
      {data.items.length === 0 && <NoResultsCompanies />}
      {data.items.length > 0 && (
        <div>
