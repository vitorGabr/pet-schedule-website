...
export function LinksSection() {
	...
	const availableRoutes = BASE_ROUTES.filter((route) => !["/about", "/contact"].includes(route.href));

	if (isNotShowSearchBar) {
		return (
			<nav className="w-full flex items-center gap-4">
				{availableRoutes.map((route) => (
					<NavLink key={route.href as string} href={route.href} isActive={pathname === route.href}>
						{route.label}
					</NavLink>
				))}
			</nav>
